...
def label_test_hyp_ref(flat_labels, flat_true_labels, new_test_name, text_file):
    ...
    for s_idx, txt in enumerate(open(text_file)):
        words = txt[:-1].decode('utf-8').strip().split()
        tag_seq = [tag_map[flat_labels[i]] for i in range(start_idx, len(words))]
        true_tag_seq = [tag_map[flat_true_labels[i]] for i in range(start_idx, len(words))]
...
'''
    Here only target is segmented with Moses using a target-source SMT system
    The segmentation has to be done in advance
'''
...
#    print("Train data: ")
#    for a_key in train_data:
#        print(a_key)
#        print(train_data[a_key][0])
#    print("Train context: ", train_contexts[0])
...
#        print(sequence_correlation_weighted(test_tags_seq_num, test_predictions_seq, verbose=True)[1])
        ...
        label_test_hyp_ref(test_predictions_word, test_tags_word, config["output_name"], config["output_test"])

#        label_test(test_predictions_word, '/export/data/varvara/marmot/marmot/experiment/final_submissions/baseline', '/export/data/varvara/corpora/wmt15_corrected/test.target', 'BASELINE')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_type", default=None, help="data type - sequential or plain")
    parser.add_argument("--bad_tagging", default=None, help="tagging -- optimistic, pessimistic or super-pessimistic")
